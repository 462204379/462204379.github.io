<template>
    <div class="upTemplate" id="template-upimg">
        <div class="nameImg">
            ...
            <div style="display: flex;margin-right: 20px;width: 200px;position: relative;align-items: center;">
                ...
                <div class="topList" @click="leftKeys = !leftKeys" style="position: absolute;
    right: 0;">
                    <div style="height: 18px;">
    ...
    export default {
        ...
        data() {
            return {
                ...
                marginTop: 20,
                username: getSessionStorage("jsonData").labelName+"-"+getSessionStorage("jsonData").sceneName+"-",
                loading: true,
                rightForm_1: true,
                rightForm_2: false,
                scale: 1,
        ...
        created() {
            this.width = Number(getSessionStorage("jsonData").width);
            this.height = Number(getSessionStorage("jsonData").height);
            this.indexAxios();
        },
        computed: {
        ...
        methods: {
            ...
            xqClick (){
                this.searchAxios();
                this.submitUp = "0";
                remark_query({modelId :getSessionStorage("jsonData").modelId})
                    .then(response => {
                        if (response.code) {
                            return false;
                        } else {
            ...
            fanhuiClick (){
                this.$confirm('是否返回，返回后系统可能不会保存您所做的更改?', '提示', {
                    ...
                    this.$router.push({ path: '/picturePs/index/template/next'});
                }).catch(() => {
                    this.$message({
                        type: 'info',
                        message: '已取消'
            ...
            indexAxios() {
                psdTemplateItem({modelId: getSessionStorage("jsonData").modelId})
                    .then(response => {
                        let fonttishi = true;
                        let itemlist = [];
                        this.loading = false;
            ...
            houtuiClick(){
                ...
                let formData = {
                    ...
                    batch: this.batch,
                    modelId: getSessionStorage("jsonData").modelId
                };
                        ...
                        that.listdata.map((item, index)=>{
                            if(that.thisAllid.indexOf(index+'') != '-1'){
                                if(item.suoding == false) {
                                    item.topBox += (e.clientY - clientY)/that.scale;
                                    item.leftBox += (e.clientX - clientX)/that.scale;
                                }
                            }
                        });
                    } else {
            ...
            okClick(val) {
                ...
                modifyPsdModel({list:itemlist,type :2})
                    .then(response => {
                        if(response.code){
                            return;
                        }
                        if(val == 0){
                            ...
                            this.$router.push({ path: "/picturePs/index/home"});
                        }
                    })
                    .catch(() => {});
            },
