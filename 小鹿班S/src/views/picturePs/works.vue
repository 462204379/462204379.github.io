<template>
    <div>
        <div class="clear"></div>
        <div class="mainDiv" id="worksMain" v-loading="loading" element-loading-text="下载中" element-loading-spinner="el-icon-loading">
            <div class="searchDiv_2">
                <div class="searchDiv_2_1">全部 ></div>
                <div class="searchDiv_2_2"></div>
                <div class="clear"></div>
            </div>
            <div class="searchDiv_3">
                ...
                <el-select v-model="search.brandName1" size="mini" @change="fileBrandChange" style="margin-left: 10px;">
                    ...
                    <el-option v-for="item in searchDiv_6_list" :key="item.val" :label="item.text" :value="item.id"></el-option>
                </el-select>
                <el-select
            ...
            <div class="searchDiv_7">
                ...
                <ul>
                    <li>
                        ...
                        <span style="margin-left: 10px;" v-if="searchSelect_7 != ''">等于</span>
            ...
            <div class="searchDiv_9">
                <el-radio v-model="photoType" label="0" @change="indexAxios">全部</el-radio>
                <el-radio v-model="photoType" label="1" @change="indexAxios">批处理</el-radio>
                <el-radio v-model="photoType" label="2" @change="indexAxios">批价格</el-radio>
            </div>
            <div class="imgList">
                ...
                <el-row :gutter="20">
                    <el-col :span="6" v-for="(item, index) in imgList" :key="index" style="position: relative;">
                        <div class="batch">批号：{{item.batch?item.batch:"[无]"}}</div>
                        <el-card :body-style="{ padding: '0px' }" shadow="hover" style="margin-bottom: 20px;text-align: center">
                            <div @click="imgToClick(item)">
                                <img id="worksImg" :src="timg" :alt="item.brandName">
                            </div>
                            <div style="padding: 10px;text-align: left">
                                <div class="bottom clearfix" style="display: flex;height: 16px;">
                                    <div style="width: 100%;overflow: hidden;white-space: nowrap;text-overflow: ellipsis;">
                                        ...
                                        <span style="color: #409EFF;font-weight: bold; "> {{ item.brandName }} </span>
                                        <span> {{ item.photoNum }}张</span>
                                    </div>
                                    <!--<i style="margin-right: 5px;" @click="removeClick(item.compoundBatch)" class="icon iconfont icon-shanchu"></i>-->
                                    <span v-if="item.photoType==1" style="float: right;" class="color01">批图片</span>
                                    <span v-else style="float: right;" class="color02">批价格</span>
                                </div>
                            </div>
                        </el-card>
                    </el-col>
        ...
        methods: {
            ...
            indexAxios() {
                ...
                myphotoList(data)
                    .then(response => {
                        ...
                        response.data.map(item=>{
                            item.select = false;
                            checkedImgsAll.push(item.compoundBatch)
                        })
            ...
            removeClick() {
                if(this.checkedImgs.length === 0){
                    this.$message.error("请选择需要删除的图片组");
                    return;
                }
                let data ={
                    batch:this.checkedImgs.join(",")
                };
                this.$confirm('此操作将删除该图片文件夹, 是否继续?', '提示', {
            ...
            pijiage(val) {
                this.$message.error("此功能暂未开放，请耐心等待！");
                /*let formList = [];
                val.map(item => {
                    if(item.select) {
            ...
            imgchange() {
                ...
                this.imgList.map(item => {
                    if (item.select) {
                        checkedImgs.push(item.compoundBatch);
                    }
                })
            ...
            daochuClick() {
                if(this.checkedImgs.length === 0){
                    this.$message.error("请选择需要下载的图片组");
                    return;
                }
                let data ={
                    compoundBatch:this.checkedImgs.join(",")
                }
                this.loading = true;
                download_photo(data)
                    .then(response => {
            ...
            imgToClick(val) {
                this.$router.push({path: "/picturePs/index/works/imges", query: {
                    compoundBatch:val.compoundBatch,
